    ...
    class Database
      class PostgresqlUser < Chef::Provider::Database::Postgresql
        ...
        def action_create
          unless exists?
            begin
              if @new_resource.password and @new_resource.superuser
              ...
              elsif @new_resource.superuser
                db("template1").query("CREATE USER \"#{@new_resource.username}\" WITH SUPERUSER")
              else
                db("template1").query("CREATE USER \"#{@new_resource.username}\"")
              end
